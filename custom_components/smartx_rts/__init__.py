...
from .controller import SmartXAPI, SmartXController


class SmartXRTSEntityFeature(IntFlag):
    """Supported features of SmartX Entities."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up SmartX RTS from a config entry."""
    api = SmartXAPI(hass, entry.entry_id, entry.data)
    controller = SmartXController(entry.entry_id, hass, api)
    hass.data.setdefault(DOMAIN, {})[entry.entry_id] = controller
    await api.get_initial()
    if not api.is_configured:
        raise ConfigEntryNotReady(
            f"Could not find SmartX RTS device with address {api.get_api_url()}"
        )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    controller: SmartXController = hass.data[DOMAIN].get(entry.entry_id)
