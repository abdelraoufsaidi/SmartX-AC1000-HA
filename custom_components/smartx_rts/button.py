"""Support for SmartX RTS device actions."""
...
from .__init__ import SmartXRTSEntityFeature
from .const import API_REBOOT, DOMAIN, EVT_CONNECTED
from .controller import SmartXController
from .entity import SmartXEntity
...
@dataclass
class SmartXButtonDescriptionMixin:
    """Mixin for entity description."""


@dataclass
class SmartXButtonDescription(
    ButtonEntityDescription, SmartXButtonDescriptionMixin
):
...
async def async_setup_entry(
    ...
    """Set up SmartX RTS update based on a config entry."""
    new_entities = []
    controller: SmartXController = hass.data[DOMAIN][config_entry.entry_id]
    v = version_parse(controller.version)
    if v.major >= 2 and v.minor >= 3 and v.micro >= 0:
        new_entities.append(
            SmartXButton(
                controller=controller,
                cfg=SmartXButtonDescription(
                    key="reboot",
    ...
    platform.async_register_entity_service(
        ...
        schema={},
        func="async_press",
        required_features=[SmartXRTSEntityFeature.REBOOT],
    )
    platform.async_register_entity_service(
        name=SVC_BACKUP,
        schema={},
        func="async_press",
        required_features=[SmartXRTSEntityFeature.BACKUP],
    )


class SmartXButton(SmartXEntity, ButtonEntity):
    """Defines a reboot entity."""

    _attr_device_class = ButtonDeviceClass.RESTART

    def __init__(
        self, controller: SmartXController, cfg: SmartXButtonDescription
    ) -> None:
