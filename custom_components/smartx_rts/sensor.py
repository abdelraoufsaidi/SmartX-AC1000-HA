"""Sensors related to SmartX-RTS-HA."""
...
@dataclass
class SmartXDiagSensorDescription(SensorEntityDescription):
    ...
    if "serverId" in data:
        if "hostname" in data:
            new_entities.append(
                SmartXDiagSensor(
                    ...
                    cfg=SmartXDiagSensorDescription(
                        key="host_name",
        ...
        if "connType" in data:
            ...
            if data["connType"] == "Wifi":
                new_entities.append(SmartXWifiStrengthSensor(controller, data))
                new_entities.append(
                    SmartXDiagSensor(
                        controller=controller,
                        cfg=SmartXDiagSensorDescription(
...
class SmartXDiagSensor(SmartXEntity, SensorEntity):
    """A diagnostic entity for the hub."""

    def __init__(
        self, controller: SmartXController, cfg: SmartXDiagSensorDescription, data
    ) -> None:
...
class SmartXWifiStrengthSensor(SmartXDiagSensor):
