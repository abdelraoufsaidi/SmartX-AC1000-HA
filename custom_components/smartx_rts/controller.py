...
class SmartXController(DataUpdateCoordinator):
    """Data coordinator/controller for receiving from SmartX_RTS."""

    def __init__(self, config_entry_id, hass: HomeAssistant, api: SmartXAPI) -> None:
    ...
    def unique_id(self) -> str:
        ...
        return f"smartx_{self.server_id}"
    ...
    def internet_available(self) -> bool:
        """Indicates whether the SmartX RTS hardware has internet access."""
    ...
    def ws_onopen(self):
        """Websocket is opened."""
        _LOGGER.debug("SmartX RTS Socket was opened")
        if self.api.is_configured:
            _LOGGER.debug("SmartX RTS Already Configured")
            data = {"event": EVT_CONNECTED, "connected": True}
            self.async_set_updated_data(data=data)
        else:
            _LOGGER.debug("SmartX RTS configuring entities")
...
class SmartXAPI:
    """API for sending data to SmartX RTS hardware device."""
    ...
    def backup_dir(self) -> str:
        ...
        return self.hass.config.path(f"SmartXRTS_{self.server_id}")
    ...
    @property
    def internet_available(self) -> bool:
        """Check to see if the SmartX RTS hardware has internet."""
    ...
    def set_firmware(self, data) -> None:
        ...
        if cver != new_ver:
            ...
            if dev := dev_registry.async_get_device(
                identifiers={(DOMAIN, f"smartx_{self.server_id}")}
            ):
    ...
    async def shade_command(self, data):
        """Send commands to SmartXRTS via PUT request."""
    ...
    async def tilt_command(self, data):
        """Send tilt commands to SmartXRTS via PUT request."""
        async with self._session.put(
    ...
    async def get_initial(self):
        """Get the initial config from SmartX RTS."""
        try:
            ...
            async with self._session.get(f"{self._api_url}{API_DISCOVERY}") as resp:
                if resp.status == 200:
                    ...
                    if not self._configured:
                        _LOGGER.debug("SmartX RTS Setting up entities")
