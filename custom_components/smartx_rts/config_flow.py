...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"title": "SmartX RTS", "server_id": "A1"}
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Configuration flow for SmartXController."""

    VERSION = 1

    def __init__(self) -> None:
        """Initialize."""
        self.controller: SmartXController
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                ...
                api = SmartXAPI(self.hass, 0, user_input)
                await api.discover()
                await self.async_set_unique_id(f"smartx_{api.server_id}")
                ...
                return self.async_create_entry(
                    ...
                    description=f"SmartX RTS {api.server_id}",
                    data=user_input,
                )
            except InvalidHost:
    ...
    async def async_step_zeroconf(
        ...
        await self.async_set_unique_id(f"smartx_{self.server_id}")
    ...
    def async_get_options_flow(
        ...
        """Get the options flow for smartx_rts."""
        return SmartXOptionsFlowHandler(config_entry)


class SmartXOptionsFlowHandler(config_entries.OptionsFlow):
    """Options flow for smartx_rts component."""
    ...
    async def async_step_init(
        ...
        """Configure options for smartx_rts."""
        errors = {}
        if user_input is not None:
            try:
