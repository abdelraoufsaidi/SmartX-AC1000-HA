"""Binary sensors related to SmartX-RTS-HA."""
            ...
            try:
                if "sunSensor" in shade:
                    if shade["sunSensor"] is True:
                        new_entities.append(SmartXSunSensor(controller, shade))
                        new_entities.append(SmartXWindSensor(controller, shade))
                    elif "shadeType" in shade:
                        match shade["shadeType"]:
                            case 3:
                                new_entities.append(
                                    SmartXWindSensor(controller, shade)
                                )
            ...
            try:
                if "sunSensor" in group:
                    if group["sunSensor"] is True:
                        new_entities.append(SmartXSunSensor(controller, group))
                        new_entities.append(SmartXWindSensor(controller, group))
            except KeyError:
                pass
    if new_entities:
...
class SmartXSunSensor(SmartXEntity, BinarySensorEntity):
...
class SmartXWindSensor(SmartXEntity, BinarySensorEntity):
    """A sun flag sensor indicating whether there is sun."""
