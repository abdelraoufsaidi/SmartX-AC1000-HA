"""Support for SmartX RTS Shades and Blinds."""
    ...
    if "serverId" in data:
        for shade in controller.api.shades:
            try:
                ...
                if "shadeType" in shade and not (
                    ...
                    new_shades.append(SmartXShade(controller, shade))
        ...
        for group in controller.api.groups:
            with contextlib.suppress(KeyError):
                new_groups.append(
                    SmartXGroup(hass=hass, controller=controller, data=group)
                )
        if new_groups:
...
class SmartXGroup(CoverGroup, SmartXEntity):
    """A grpi[] that is associated with a controller."""

    def __init__(
        self, hass: HomeAssistant, controller: SmartXController, data
    ) -> None:
        """Initialize a group."""
        SmartXEntity.__init__(self=self, controller=controller, data=data)
        self._hass = hass
...
class SmartXShade(SmartXEntity, CoverEntity):
    """A shade that is associated with a controller."""

    def __init__(self, controller: SmartXController, data) -> None:
    ...
    def _handle_state_command(self, data) -> None:
        ...
        self.hass.bus.async_fire("smartx_rts_event", bus_data)
        if upd:
            self.async_write_ha_state()
